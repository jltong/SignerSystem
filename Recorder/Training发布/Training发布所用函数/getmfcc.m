...
function [mfccc,FrameSize,framelocation,energy]=getmfcc(speechname,SpeakerName,dimension)%对某一个文件夹的某一个文件进行提取mfcc参数。返回mfcc参数ccc,帧数sizec，
	...
	x=0.3*wavx;% a mapping of the sampled data
	
    xtemp=double(x); %return the double-precision value of x
    
    xxtemp=enframe(xtemp,framelength,framelength/2);% 语音信号分帧  对x 256点分为一帧 按行存储
	%xxtemp store the splited frame that is one per row 
	
    xxtempE=xxtemp.^2;
	
    xxtempEn=sum(xxtempE,2);%sum every row
    ...
    xxtempZ=xxtempZ-2*xxtempzm;%短时过零率低
    [xline,xrow]=size(xxtemp);
    Discard=0;
    j=1;
	%process the discardable part
...
%melhigh : the maxfrequency in the mel scale
e=2.718281828459045;
each=melhigh/bankm;
...
dctcoef=zeros(dimension/2,bankm);%DCT coefficient
